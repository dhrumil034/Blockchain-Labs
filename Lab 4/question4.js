...
em.on('logData',function(data){
    bkc_logging_using_to_address(data);
});
...
function client(){
  ...
  this.execute = function() {
    ...
    console.log("user A log-in completed");
    server1.user_Login("userB","pwd456");
    console.log("user B log-in completed");

    server1.user_Logout("userA");
    console.log("user A log-out completed");
    server1.user_Logout("userB"); 
    console.log("user B log-out completed");
  }  
}
...
function bkc_logging_using_data(str){ 
...
//case 2
/*Function to generate hex encoded value for input string & sending transaction to blockchain for logging puropse*/
function bkc_logging_using_to_address(str){ 
    let str_hex = web3.utils.toHex(str);  
